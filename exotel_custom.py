...
class ExotelFrameSerializer(FrameSerializer):
    ...
    async def serialize(self, frame: Frame) -> str | bytes | None:
        ...
        if isinstance(frame, StartInterruptionFrame):
            ...
            data = frame.audio
            # Resample if needed
            audio_data, _ = audioop.ratecv(data, 2, 1, frame.sample_rate, self._exotel_sample_rate, None)
            
            # ✅ Correct: Encode to base64
            payload = base64.b64encode(audio_data).decode("ascii")  # safely converted to JSON-friendly string
    ...
    async def deserialize(self, data: str | bytes) -> Frame | None:
        ...
        if message["event"] == "media":
            payload_base64 = message["media"]["payload"]
            payload = base64.b64decode(payload_base64)  # ✅ correct

            # Convert Exotel's 8kHz to your internal sample rate
            audio_data, _ = audioop.ratecv(payload, 2, 1, self._exotel_sample_rate, self._sample_rate, None)
